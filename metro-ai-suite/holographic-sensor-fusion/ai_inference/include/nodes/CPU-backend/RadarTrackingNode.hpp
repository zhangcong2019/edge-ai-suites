...
#ifndef __RADAR_TRACKING_NODE_H_
#define __RADAR_TRACKING_NODE_H_
...
class RadarTrackingNode : public hva::hvaNode_t {
  public:
    RadarTrackingNode(std::size_t totalThreadNum);

    virtual ~RadarTrackingNode();
...
class RadarTrackingNodeWorker : public hva::hvaNodeWorker_t {
  public:
    RadarTrackingNodeWorker(hva::hvaNode_t *parentNode);

    virtual ~RadarTrackingNodeWorker();

    void init() override;

    virtual const std::string nodeClassName() const
    {
        return "RadarTrackingNodeWorker";
    };

    /**
     * @brief Called by hva framework for each frame, Run radar tracking and
