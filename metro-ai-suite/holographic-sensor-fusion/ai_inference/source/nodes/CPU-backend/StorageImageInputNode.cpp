...
#include "nodes/CPU-backend/StorageImageInputNode.hpp"
...
StorageImageInputNode::StorageImageInputNode(std::size_t totalThreadNum) : baseMediaInputNode(totalThreadNum) {
    transitStateTo(hva::hvaState_t::configured);
}

std::shared_ptr<hva::hvaNodeWorker_t> StorageImageInputNode::createNodeWorker() const {
    return std::shared_ptr<hva::hvaNodeWorker_t>(new StorageImageInputNodeWorker((hva::hvaNode_t*)this));
}

StorageImageInputNodeWorker::StorageImageInputNodeWorker(hva::hvaNode_t* parentNode)
        :baseMediaInputNodeWorker(parentNode){
}

void StorageImageInputNodeWorker::init(){
 ...
 */
readMediaStatus_t StorageImageInputNodeWorker::processMediaContent(const std::string& buf, std::string& content, 
                                                     HceDatabaseMeta& meta, hva::hvaROI_t& roi, unsigned order)
{
    meta.mediaUri = buf;
    ...
    if(!attrib.empty()){
      ...
      try {
            ...
            HVA_ERROR("Failed to read json media attrib: %s", boost::diagnostic_information(e).c_str());
            return readMediaStatus_t::Failure;
...
HVA_ENABLE_DYNAMIC_LOADING(StorageImageInputNode, StorageImageInputNode(threadNum))
