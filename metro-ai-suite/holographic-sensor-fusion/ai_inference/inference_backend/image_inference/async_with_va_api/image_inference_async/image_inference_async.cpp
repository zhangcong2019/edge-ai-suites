...
#include "feature_toggling/environment_variable_options_reader.h"
...
#include "feature_toggling/runtime_feature_toggler.h"
...
ImageInferenceAsync::ImageInferenceAsync(const InferenceBackend::InferenceConfig &config,
                                         hce::ai::inference::ContextPtr vadpy_context, ImageInference::Ptr inference)
    : _inference(inference) {
    const auto &pre_process_config = config.at(KEY_PRE_PROCESSOR);
    if (!Utils::checkAllKeysAreKnown({KEY_VAAPI_THREAD_POOL_SIZE, KEY_VAAPI_FAST_SCALE_LOAD_FACTOR, KEY_SCALE_FACTOR},
                                     pre_process_config)) {
