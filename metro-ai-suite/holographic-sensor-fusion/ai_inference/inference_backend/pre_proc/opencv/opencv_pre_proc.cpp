...
void OpenCV_VPP::Convert(const Image &raw_src, Image &dst, const InputImageLayerDesc::Ptr &pre_proc_info,
    ...
    try {
        ...
        if (needCustomImageConvert(pre_proc_info)) {
            ...
            if (src_mat_image.size().width > dst.width || src_mat_image.size().height > dst.height) {
                dst_mat_image = ResizeMat(src_mat_image, dst.height, dst.width);
            } else {
                dst_mat_image = PadMat(src_mat_image, dst.height, dst.width, cv::Scalar(0, 0, 0));
            }
        }

        if (dst.format == FOURCC_RGBP_F32) {
            dst_mat_image.convertTo(dst_mat_image, CV_MAKETYPE(CV_32F, dst_mat_image.channels()));
        }
        ...
        if (make_planar) {
            ...
            if (dst.format == FOURCC_RGBP_F32) {
                cv::Mat dst_mat(safe_convert<int>(dst.height), safe_convert<int>(dst.width),
                            CV_MAKE_TYPE(CV_32F, channels_count), reinterpret_cast<float*>(dst.planes[0]), dst.stride[0] * sizeof(float));
