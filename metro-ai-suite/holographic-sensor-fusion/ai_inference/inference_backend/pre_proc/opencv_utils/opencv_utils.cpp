...
int CreateMat(uint8_t *const *planes, uint32_t src_width, uint32_t src_height, int format, const uint32_t *stride,
    ...
    case FOURCC_RGBP_F32: {
        cv::Mat channelR =
            cv::Mat(safe_convert<int>(src_height), safe_convert<int>(src_width), CV_32FC1, planes[0], stride[0] * sizeof(float));
        cv::Mat channelG =
            cv::Mat(safe_convert<int>(src_height), safe_convert<int>(src_width), CV_32FC1, planes[1], stride[1] * sizeof(float));
        cv::Mat channelB =
            cv::Mat(safe_convert<int>(src_height), safe_convert<int>(src_width), CV_32FC1, planes[2], stride[2] * sizeof(float));
        std::vector<cv::Mat> channels{channelB, channelG, channelR};
        cv::merge(channels, dst);
        return FOURCC_BGRP;
    }
...
cv::Mat PadMat(const cv::Mat &orig_image, const size_t height, const size_t width, const cv::Scalar &value) {
    cv::Mat padded_image;
    int top = (height - orig_image.rows) / 2;
    int bottom = height - orig_image.rows - top;
    int left = (width - orig_image.cols) / 2;
    int right = width - orig_image.cols - left;

    if (top < 0 || bottom < 0 || left < 0 || right < 0) {
        return orig_image;
    }
    ITT_TASK("cv::copyMakeBorder");
    cv::copyMakeBorder(orig_image, padded_image, top, bottom, left, right, cv::BORDER_CONSTANT, value);
    return padded_image;
}
