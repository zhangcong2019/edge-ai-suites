...
int main(int argc, char** argv){
    ...
    try
    {
        ...
        //
        // load data according to the media_type
        // "image" || "image_raw": read image files from specific directory
        // "video": read video file, only support for single file
        //
        if (!checkPathExist(data_path)) {
            HVA_ERROR("path not exists: %s", data_path.c_str());
            HVA_ASSERT(false);
        }
        std::vector<std::string> mediaVector;
        if (media_type == "image" || media_type == "image_raw") {
            // read files from directory
            // Recursive reading is not supported
            HVA_DEBUG("testMultiStreamAiPipeline load images from folder: %s", data_path.c_str());
            getAllFiles(data_path, mediaVector, ".jpg");
        }
        else if (media_type == "video") {
            if (checkRegularFile(data_path)) {
                HVA_DEBUG("testMultiStreamAiPipeline load video file: %s", data_path.c_str());
                mediaVector.push_back(data_path);
            }
            else {
                HVA_DEBUG("testMultiStreamAiPipeline load videos from folder: %s", data_path.c_str());
                getAllFiles(data_path, mediaVector);
            }

        }
        else {
            HVA_ERROR("testMultiStreamAiPipeline receive unknow media type: %s", media_type.c_str());
            HVA_ASSERT(false);
        }
        HVA_INFO("mediaVector size is: %d", mediaVector.size());
        if (mediaVector.size() == 0) {
            HVA_ERROR("empty data loaded!");
            HVA_ASSERT(false);
        }
        ...
        for(const auto& item: mediaVector){
            ...
            //
            // load image-data according to the media_type
            // "image": image file path
            // "image_raw": base64_encoded image content
            //
            if (media_type == "image_raw") {
                /* mode: Raw */
                std::string imageContent;
                readJpgToCharBuffer(item, imageContent);
                if (imageContent.size() == 0) {
                    HVA_ERROR("empty image content!");
                    HVA_ASSERT(false);
                }
                inputs.push_back(composeMediaInputWithROI(imageContent, 0, 0, 0, 0));
            } else {
                /* mode: LocalFile */
        ...
        if (cross_stream_num > 1) {
            ...
            if (inputs.size() < cross_stream_num) {
                for (int i = inputs.size(); i<cross_stream_num; i++) {
        ...
        for(unsigned cnt =0; cnt < repeats; ++cnt){
            for (unsigned i = 0; i < threads; ++i) {
                ...
                int segNum = std::floor(inputs.size() / cross_stream_num);
                for (unsigned streamId = 0; streamId < cross_stream_num; ++streamId) {

                    // get piece for cur streamId
                    int beginIndex = (int)streamId * segNum;
                    int endIndex = streamId == (cross_stream_num - 1) ? (int)inputs.size() : beginIndex + segNum;
                    std::vector<std::string> curInputs(inputs.begin() + beginIndex, inputs.begin() + endIndex);
