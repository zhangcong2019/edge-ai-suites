...
Image ApplyCrop(const Image &src) {
    ...
    case InferenceBackend::FOURCC_RGBP_F32: {
        dst.planes[0] = src.planes[0] + (src.rect.y * src.stride[0] + src.rect.x) * sizeof(float);
        dst.planes[1] = src.planes[1] + (src.rect.y * src.stride[1] + src.rect.x) * sizeof(float);
        dst.planes[2] = src.planes[2] + (src.rect.y * src.stride[2] + src.rect.x) * sizeof(float);
        break;
    }
