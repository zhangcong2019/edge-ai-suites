...
#ifndef __LOCAL_MEDIA_SENSOR_INPUT_NODE_H_
#define __LOCAL_MEDIA_SENSOR_INPUT_NODE_H_
...
struct MediaDatasetField_t {
  std::string imageContent;
  size_t imageSize = 0;
};

class LocalMediaSensorInputNode : public hva::hvaNode_t{
public:

    struct InpustSensorIndices_t {
        int mediaIndex = 0;         // output port index for media input
    };

    LocalMediaSensorInputNode(std::size_t totalThreadNum);

    ~LocalMediaSensorInputNode() = default;
    ...
    */
    ...
    virtual const std::string nodeClassName() const override{ return "LocalMediaSensorInputNode";};
...
class LocalMediaSensorInputNodeWorker : public hva::hvaNodeWorker_t{
public:
    LocalMediaSensorInputNodeWorker(hva::hvaNode_t* parentNode, const LocalMediaSensorInputNode::InpustSensorIndices_t &sensorIndices,
...
private:
    LocalMediaSensorInputNode::InpustSensorIndices_t m_sensorIndices;
    size_t m_inputCapacity;
    size_t m_stride;
...
#endif //#ifndef __LOCAL_MEDIA_SENSOR_INPUT_NODE_H_
