...
#include "nodes/CPU-backend/LocalMediaSensorInputNode.hpp"
...
LocalMediaSensorInputNode::LocalMediaSensorInputNode(std::size_t totalThreadNum):hva::hvaNode_t(1, 1, totalThreadNum) {
...
*/
hva::hvaStatus_t LocalMediaSensorInputNode::configureByString(const std::string& config) {
...
std::shared_ptr<hva::hvaNodeWorker_t> LocalMediaSensorInputNode::createNodeWorker() const{
    return std::shared_ptr<hva::hvaNodeWorker_t>(new LocalMediaSensorInputNodeWorker((hva::hvaNode_t*)this, m_sensorIndices, m_inputCapacity, m_stride, m_frameRate, m_controlType));
} 


LocalMediaSensorInputNodeWorker::LocalMediaSensorInputNodeWorker(hva::hvaNode_t* parentNode, 
        const LocalMediaSensorInputNode::InpustSensorIndices_t &sensorIndices, const size_t &inputCapacity, const size_t &stride, const float &frameRate, const std::string &controlType):
...
void LocalMediaSensorInputNodeWorker::process(std::size_t batchIdx){
    ...
    if (vecBlobInput.size() != 0) {
        ...
        HVA_DEBUG("Local MediaSensor Input node %d on frameId %d", batchIdx, inBlob->frameId);

        getParentPtr()->emitEvent(hvaEvent_PipelineLatencyCapture, &inBlob->frameId);
        std::shared_ptr<hva::timeStampInfo> inputIn =
            std::make_shared<hva::timeStampInfo>(inBlob->frameId, "InputIn");
        getParentPtr()->emitEvent(hvaEvent_PipelineTimeStampRecord, &inputIn);
        ...
        if (comingInputs.size() == 0 ) {
            HVA_ERROR("Local MediaSensor Input node %d received invalid inputs length: %d on frame %d", 
                       batchIdx, comingInputs.size(), inBlob->frameId);
        ...
        for (int inputIdx = 0; inputIdx < comingInputs.size(); inputIdx ++) {
            std::shared_ptr<hva::timeStampInfo> mediaInputIn =
                std::make_shared<hva::timeStampInfo>(inBlob->frameId, "mediaInputIn");
            getParentPtr()->emitEvent(hvaEvent_PipelineTimeStampRecord, &mediaInputIn);
            // the last request input will be taged as end
            bool isEnd = (inputIdx >= comingInputs.size() - 1);
            ...
            // getParentPtr()->emitEvent(hvaEvent_PipelineLatencyCapture, &blob->frameId);

            // if (blob->frameId < 5) {
            //     std::shared_ptr<hva::timeStampInfo> inputIn = std::make_shared<hva::timeStampInfo>(blob->frameId, "InputNodeIn");
            //     getParentPtr()->emitEvent(hvaEvent_PipelineTimeStampRecord, &inputIn);
            // }

            getLatencyMonitor().startRecording(blob->frameId,"reading file");
            std::string path = comingInputs[inputIdx];
            // read binary data
            std::fstream fs;
            ...
            getLatencyMonitor().stopRecording(blob->frameId,"reading file");
            ...
            getLatencyMonitor().startRecording(blob->frameId,"send output");
            sendOutput(blob, 0, std::chrono::milliseconds(0));
            getLatencyMonitor().stopRecording(blob->frameId,"send output");

            // if (blob->frameId < 5) {
            //     std::shared_ptr<hva::timeStampInfo> inputOut = std::make_shared<hva::timeStampInfo>(blob->frameId, "InputNodeOut");
            ...
            std::shared_ptr<hva::timeStampInfo> mediaInputOut =
            std::make_shared<hva::timeStampInfo>(inBlob->frameId, "mediaInputOut");
            getParentPtr()->emitEvent(hvaEvent_PipelineTimeStampRecord, &mediaInputOut);
        }
        std::shared_ptr<hva::timeStampInfo> inputOut =
        std::make_shared<hva::timeStampInfo>(inBlob->frameId, "InputNodeOut");
        getParentPtr()->emitEvent(hvaEvent_PipelineTimeStampRecord, &inputOut);
    }
 ...
 * @param void
 */
unsigned LocalMediaSensorInputNodeWorker::fetch_increment() { return m_ctr++; }
 ...
 */
void LocalMediaSensorInputNodeWorker::sendEmptyBlob(hva::hvaBlob_t::Ptr blob,
                                            bool isEnd,
...
HVA_ENABLE_DYNAMIC_LOADING(LocalMediaSensorInputNode, LocalMediaSensorInputNode(threadNum))
