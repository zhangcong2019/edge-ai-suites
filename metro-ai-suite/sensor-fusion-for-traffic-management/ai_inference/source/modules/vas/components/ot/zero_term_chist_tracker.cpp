...
int32_t ZeroTermChistTracker::TrackObjects(const cv::Mat &mat, const std::vector<Detection> &detections,
    ...
    if (input_image_format_ == hce::ai::inference::ColorFormat::NV12 
        || input_image_format_ == hce::ai::inference::ColorFormat::I420) {
    ...
    if (detections.size() > 0) {
        ...
        for (const auto &detection : detections) {
            ...
            } else if (input_image_format_ == hce::ai::inference::ColorFormat::BGR) {
                cv::Mat roi_patch = mat(detection.rect & image_boundary);
                rgb_hist_.Compute(roi_patch, &rgb_feature);
            } else if (input_image_format_ == hce::ai::inference::ColorFormat::BGRX) {
                cv::Mat roi_patch = mat(detection.rect & image_boundary);
                rgb_hist_.ComputeFromBgra32(roi_patch, &rgb_feature);
            } else if (input_image_format_ == hce::ai::inference::ColorFormat::I420) {
    ...
    for (int32_t t = 0; t < n_tracklets; ++t) {
        ...
        if (t_associated_d_index[t] >= 0) {
            ...
            tracklet->label_name = detections[associated_d_index].class_label_name;
            tracklet->confidence = detections[associated_d_index].confidence;
    ...
    for (int32_t d = 0; d < static_cast<int32_t>(detections.size()); ++d) {
        if (d_is_associated[d] == false) {
            ...
            tracklet->label_name = detections[d].class_label_name;
            tracklet->confidence = detections[d].confidence;
