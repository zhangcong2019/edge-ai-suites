#!/usr/bin/env bash
# Apply the Intel patches on top of the pristine FAST_LIO submodule
...
if [[ ! -e .git ]]; then
  echo "FAST_LIO submodule is not initialized. Run:" >&2
  echo "  git submodule update --init --recursive ${FASTLIO_SRC}" >&2
  exit 1
fi

if [[ ! -e include/ikd-Tree/ikd_Tree.cpp ]]; then
  echo "FAST_LIO's nested ikd-Tree submodule is not initialized. Run:" >&2
  echo "  git submodule update --init --recursive ${FASTLIO_SRC}" >&2
  exit 1
fi

if [[ -n "$(git status --porcelain)" ]]; then
  echo "FAST_LIO checkout has local changes; refusing to apply patches on top of a dirty tree." >&2
...
for patch in "${PATCH_DIR}"/0*.patch; do
  # git format-patch wraps long Subject: headers onto RFC2822-style
  # continuation lines (a following line starting with whitespace) - a
  # plain `grep -m1 '^Subject: '` silently truncates any subject long
  # enough to wrap, which breaks the already-applied lookup below for
  # exactly those patches. Join continuation lines back in.
  subject="$(awk '
    /^Subject: / { sub(/^Subject: (\[PATCH[^]]*\] )?/, ""); s=$0; capturing=1; next }
    capturing && /^[ \t]/ { sub(/^[ \t]+/, " "); s=s $0; next }
    capturing { exit }
    END { print s }
  ' "${patch}")"
