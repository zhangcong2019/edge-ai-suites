#!/usr/bin/env python3
"""Subscribe to point_lio's odometry topic and write it out in TUM format.

point_lio's ROS2 port has no built-in TUM trajectory export - it only
publishes nav_msgs/Odometry (on /aft_mapped_to_init, unlike FAST_LIO2's
/Odometry - see env.sh's POINTLIO_ODOM_TOPIC). This is the small piece that
fills that gap so evaluate_rmse.sh has an estimated trajectory to compare
against ground truth.

Usage:
  record_odometry_tum.py --topic /aft_mapped_to_init --out est_tum.txt
"""
import argparse
import signal
import sys
