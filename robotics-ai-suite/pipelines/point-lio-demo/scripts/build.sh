#!/usr/bin/env bash
# Build point_lio with colcon, building its livox_ros_driver2 dependency into
# the workspace on first run (point_lio's CMakeLists.txt/package.xml depend
...
mkdir -p "${WS_DIR}/src"
ln -sfn "${POINTLIO_SRC}" "${WS_DIR}/src/point_lio"
...
cd "${WS_DIR}"
colcon build --packages-select point_lio "${CMAKE_ARGS[@]}"
