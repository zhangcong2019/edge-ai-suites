#!/usr/bin/env bash
# Apply the Intel patches on top of the pristine Point-LIO submodule
...
if [[ ! -e .git ]]; then
  echo "Point-LIO submodule is not initialized. Run:" >&2
  echo "  git submodule update --init ${POINTLIO_SRC}" >&2
  exit 1
fi

if [[ -n "$(git status --porcelain)" ]]; then
  echo "Point-LIO checkout has local changes; refusing to apply patches on top of a dirty tree." >&2
