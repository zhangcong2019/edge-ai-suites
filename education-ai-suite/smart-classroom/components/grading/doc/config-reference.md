...
| `title_patterns` | list (regex) | see config | Patterns that identify a section heading (e.g. `一、`, `SECTION I`). |
...
| `ordinal_pattern` | str (regex) | see config | Extracts the leading ordinal from a heading; group 1 is used to match a section to its rubric block. |
...
| `marker_pattern` | str (regex) | see config | Marks the header block in the rubric (`[HEADER]` or `【卷头信息】`). |
...
- **VLM Parameters**: `image.dpi`, `vlm.temperature`
- **Grading Pace**: `watch.poll_interval`, `watch.stable_checks`, `watch.idle_timeout`
- **Layout Detection**: `detection_service.min_score`, `detection_service.expand_margin`, `detection_service.iou_threshold`

Other keys are edited directly in `config.yaml`.
